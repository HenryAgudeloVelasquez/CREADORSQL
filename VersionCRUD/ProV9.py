...
def crear_crud_sql():
    ...
    def guardar_archivos():
        ...
        try:
            ...
            nombre_general = nombre_general_entry.get().strip()  # Capturar el nombre general
            procedimientos = {
    ...
    for i, proc in enumerate(procedimientos_labels, start=4):  # Iniciar en row=4
    ...
    tk.Label(ventana, text="Columnas:").grid(row=10, column=0, padx=5, pady=5)  # Ajustar row
    columnas_frame = tk.Frame(ventana)
    columnas_frame.grid(row=11, column=0, columnspan=3, padx=5, pady=5)  # Ajustar row
    ...
    tk.Button(ventana, text="Agregar columna", command=agregar_columna).grid(row=12, column=1, padx=5, pady=5)  # Ajustar row

    # --- Vista previa de la tabla ---
    tk.Label(ventana, text="Vista previa de la tabla:").grid(row=13, column=0, padx=5, pady=5)  # Ajustar row
    tabla_text = tk.Text(ventana, height=10)
    tabla_text.grid(row=14, column=0, columnspan=3, padx=5, pady=5)  # Ajustar row

    # --- Botón guardar ---
    tk.Button(ventana, text="Guardar", command=guardar_archivos).grid(row=15, column=1, padx=5, pady=5)  # Ajustar row
