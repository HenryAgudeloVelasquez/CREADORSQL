...
def crear_crud_sql():
    ...
    def guardar_archivos():
        ...
        try:
            ...
            crear_procedimientos_almacenados(ubicacion, nombre_tabla, columnas, fecha_actual, **procedimientos, alias=alias)
    ...
    for i, proc in enumerate(procedimientos_labels, start=3):
    ...
    tk.Label(ventana, text="Columnas:").grid(row=9, column=0, padx=5, pady=5)
    columnas_frame = tk.Frame(ventana)
    columnas_frame.grid(row=10, column=0, columnspan=3, padx=5, pady=5)
    ...
    tk.Button(ventana, text="Agregar columna", command=agregar_columna).grid(row=11, column=1, padx=5, pady=5)

    # --- Vista previa de la tabla ---
    tk.Label(ventana, text="Vista previa de la tabla:").grid(row=12, column=0, padx=5, pady=5)
    tabla_text = tk.Text(ventana, height=10)
    tabla_text.grid(row=13, column=0, columnspan=3, padx=5, pady=5)

    # --- Botón guardar ---
    tk.Button(ventana, text="Guardar", command=guardar_archivos).grid(row=14, column=1, padx=5, pady=5)
...
def crear_procedimientos_almacenados(ubicacion,
                                    ...
                                    fecha_actual,
                                    alias="", 
                                    **procedimientos):  # Se usa **kwargs para los nombres de procedimientos
    """Crea los archivos SQL para el CRUD con nombres personalizados."""

    # --- ADD ---
    nombre_proc_add = f"{alias}{procedimientos.get('ADD') or 'Insertar'}"  # Se concatena el alias
    contenido_add = f"""
-- =============================================
...
END
    ...
    # --- EDIT ---
    nombre_proc_edit = f"{alias}{procedimientos.get('EDIT') or 'Actualizar'}"
    contenido_edit = f"""
-- =============================================
-- Author:        Henry Agudelo
-- Create date: {fecha_actual}
...
END
    ...
    # --- DELETE ---
    nombre_proc_delete = f"{alias}{procedimientos.get('DELETE') or 'Eliminar'}"
    contenido_delete = f"""
-- =============================================
-- Author:        Henry Agudelo
-- Create date: {fecha_actual}
...
END
    ...
    # --- GET ---
    nombre_proc_get = f"{alias}{procedimientos.get('GET') or 'ObtenerPorId'}"
    contenido_get = f"""
-- =============================================
-- Author:        Henry Agudelo
-- Create date: {fecha_actual}
...
END
    ...
    # --- GETLIST ---
    nombre_proc_getlist = f"{alias}{procedimientos.get('GETLIST') or 'ObtenerLista'}"
    contenido_getlist = f"""
-- =============================================
-- Author:        Henry Agudelo
-- Create date: {fecha_actual}
...
END
    ...
    # --- GETLISTALL ---
    nombre_proc_getlistall = f"{alias}{procedimientos.get('GETLISTALL') or 'ObtenerTodos'}"
...
END
    """
    guardar_archivo(ubicacion, f"{nombre_proc_getlistall}.sql",
                    contenido_getlistall)
